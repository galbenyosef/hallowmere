...
| M3 | 1.3 | sonnet | effects-factory.js, enemy-spawner.js | dist/main.js, dist/game-context.js (WIRED_SLOTS), 2 new, tests/{effects-factory,enemy-spawner}.test.mjs | M2 | review | codex/effects-spawner-96498203-006a | 5606 | — | base 6695720; orchestrator static review clean: all 11 functions verbatim (normalised-whitespace equality vs main), call sites reconciled (ringEffect/removeObject/updateEnemyBar each −1 in main.js = the internal calls that moved with their callers), main.js diff = 2 imports + 1 wiring line + createEnemyVisuals import dropped + X→ctx.X; WIRED_SLOTS +11; 12 new tests (51+22 asserts) pass with core-purity + automation-surface; edit uncommitted, compare-shots 11/11 within threshold (orchestrator-run, max 0.0820%); perf within noise: same-conditions A/B (main 6.80/8.50/9.50 idle, 7.30/10.40/12.90 walking vs M3 7.70/9.80/12.20, 8.10/11.80/13.30; p50 within 13%, idle p99 outlier coincides with a GC, heap delta −18 MB; no per-frame path changed); commits pending |
