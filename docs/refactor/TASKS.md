...
| T1-9 | 1.3 | haiku | Dead code | dist/campaign.js VILLAGES, unused exports (re-verified), app-icon.png → docs/assets | — | changes-requested | codex/dead-code-ade7a759-c49f | 5806 | 62aa3ac | VILLAGES removed; 4 exports dropped (DEFAULT_SORCERER_APPEARANCE, FOOD_COOLDOWN, MUSIC_FADE_SECONDS, colorWarden); 5 kept (consumed by golden/parity tests); app-icon.png → docs/assets; both doc links broken → fix requested; openMainMenu: no in-game caller, exercised only by game-mode-choice test → M9 decides; base 104262a; must not touch main.js (M2 owns it); openMainMenu deferred to M9 |
