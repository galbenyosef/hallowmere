...
| T0-8 | 0C | orchestrator | Browser scripts must exit when done | scripts/{screenshot,perf-browser}.mjs | — | review | codex/screenshot-exit-8cd53587-da92 | 5307 | f13896a | root cause of the capture deadlock: Chrome helpers inherit the stderr pipe and keep node alive after a retry; fix = destroy stderr + unref after the endpoint is read, explicit exit after stdout drains in both CLIs; --list smoke + build green; first Chrome run under the fix = T2-6 (merged the branch); merge at Gate 1.4
...
| P3 | 3.1 | sonnet | light-source array | dist/combat-effects.js | G2.6 | changes-requested | codex/light-source-array-d73101a9-fcab | 5489 | 90759d9 | dist change approved (strict-greater top-two scan == stable sort positions 0/1 for all tie patterns; verified by orchestrator); parity test compared a mirror of the fast path (mirror verified verbatim today, 13/13 lines) — asked for a source-containment guard + a parity run against the real update() via the public API (bolts + external lights) |
