...
| M2 | 1.2 | sonnet | model-kit.js, icon-atlas.js | dist/main.js, dist/game-context.js (WIRED_SLOTS), 2 new, 5 appearance tests, tests/{icon-atlas,model-kit}.test.mjs | M1 | review | codex/model-kit-icon-atlas-1fa61f22-cd7f | 5589 | c10d089..0ab66ff | static review clean: paintIcons(document) at the same point in module order; cloneModel wired via createModelCache + WIRED_SLOTS; 5 tests migrated with identical assert/test counts; new icon-atlas + model-kit tests; awaiting compare-shots/perf evidence; base 7453fc3 |
