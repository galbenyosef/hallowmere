import {mapFor,sameMap} from '../dist/regions.js';
import {initializeRegions,refreshGates,discoverEntrances,returnToCheckpoint,regionCommand,regionInteractions,regionalKill} from './region-campaign.mjs';
import {updateRegionEnemy,stepRegionHazards} from './region-combat.mjs';
...
export class World {
 ...
 join(token){
  ...
  }else {returnToCheckpoint(this,p);}
 ...
 command(id,m){
  ...
  if(['travel','objective','checkpoint','cache'].includes(m.type))return regionCommand(this,p,m);
  ...
  if(m.type==='service'){
   const npc=NPCS.find(n=>n.id===m.npcId);if(!npc||!sameMap(p,npc)||distance(p,npc)>2.8||!hasLineOfSight(p,npc,this.obstacles))return false;
 ...
 forage(p,id){
  ...
  if(!patch||!sameMap(p,patch)||this.time<(p.forageReadyAt[id]||0)||distance(p,patch)>2.8||!hasLineOfSight(p,patch,this.obstacles))return false;
 ...
 damageEnemy(e,amount,magic=false,color,visual){
  ...
  for(const p of this.connected()){
   ...
   drops.forEach((d,i)=>{const a=i/drops.length*Math.PI*2;let point={x:e.x+Math.cos(a)*.65,z:e.z+Math.sin(a)*.65};if(pointBlocked(point,this.obstaclesFor(e),.3))point={x:e.x,z:e.z};p.loot.push({...d,...point,mapId:e.mapId});});
  }
 ...
 obstaclesFor(entity){return (entity.mapId||'overworld')==='overworld'?this.obstacles:this.mapLayouts[entity.mapId]?.obstacles||[];}
 boundsFor(entity){return mapFor(entity.mapId).bounds;}
 move(entity,dx,dz,radius=.42){const next=resolveMove(entity,dx,dz,this.obstaclesFor(entity),radius,this.boundsFor(entity));entity.moving=distance(entity,next)>.001;Object.assign(entity,next);}
 step(dt=.05){
  ...
  stepRegionHazards(this,dt);
  this.projectiles=this.projectiles.filter(b=>{
   if(!b.hostile&&b.ownerId){const owner=this.players.get(b.ownerId);if(!owner?.connected||owner.state.ended||!sameMap(owner,b))return false;}
   ...
   if(!hasLineOfSight(previous,b,this.obstaclesFor(b),.08))return false;
   if(b.hostile){for(const p of players)if(sameMap(p,b)&&!p.state.ended&&!isSanctuary(p)&&segmentHitsCircle(previous,b,p,.48)){this.damagePlayer(p,b.damage);return false;}}
   ...
   else for(const e of this.enemies)if(sameMap(e,b)&&e.hp>0&&!b.hitIds?.includes(e.id)&&segmentHitsCircle(previous,b,e,(ENEMY_TYPES[e.type].boss||e.type==='boss'?1.05:.62)+(b.skill?.hitRadius||0))&&(!b.skill?.hitRadius||hasLineOfSight(b,e,this.obstaclesFor(b),.08))){const owner=this.players.get(b.ownerId);if(b.skill&&owner){hitEnemy(this,owner,e,b.skill,b.damage);b.hitIds.push(e.id);if(--b.remainingHits<=0)return false;}else{this.damageEnemy(e,b.damage,true);return false;}}
   return b.life>0;
  });
 }
 ...
 seek(e,target,dt){
  let goal=target;if(!hasLineOfSight(e,target,this.obstaclesFor(e),.43)){
 ...
 snapshot(id,afterEvent=0){const p=this.players.get(id);return {type:'snapshot',worldId:this.id,tick:this.tick,time:this.time,ack:p.lastSeq,mapId:p.mapId,interactions:regionInteractions(this,p),brokenCover:this.obstaclesFor(p).filter(o=>o.destructible&&o.disabled).map(o=>o.id),hazards:this.hazards.filter(h=>sameMap(h,p)).map(({hitIds,damage,...h})=>h),state:this.viewState(p),you:id,
  players:this.connected().map(q=>({id:q.id,mapId:q.mapId,slot:q.slot,color:q.color,x:q.x,z:q.z,angle:q.angle,moving:q.moving,hp:q.state.hp,maxHp:q.state.maxHp,ended:q.state.ended,dodge:q.dodge,classId:q.state.classId,appearanceId:q.state.appearanceId,rooted:q.rootUntil>this.time,speed:q.rootUntil>this.time?0:classFor(q.state).speed,shield:q.state.shield||0,concealed:q.state.concealed||0,zone:zoneAt(q)})),
