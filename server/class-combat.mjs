...
import {abilitiesFor,classFor,classColor,applyClass,weaponForClass} from '../dist/classes.js';
import {useAbility,distance,withinArc,hasLineOfSight,pointBlocked} from '../dist/combat.js';
import {isSanctuary,WORLD_BOUNDS,ITEM_TEMPLATES} from '../dist/campaign.js';
...
function centerFor(world,p,m,skill,angle){
 ...
 const center={x:p.x+dx*scale,z:p.z+dz*scale};
 if(center.x<WORLD_BOUNDS.minX||center.x>WORLD_BOUNDS.maxX||center.z<WORLD_BOUNDS.minZ||center.z>WORLD_BOUNDS.maxZ||pointBlocked(center,world.obstacles,.1)||!hasLineOfSight(p,center,world.obstacles,.1))return null;
 return center;
}
export function hitEnemy(world,p,e,skill,amount){
 ...
 if(e.hp<=0)return;
 if(skill.root){e.rootUntil=Math.max(e.rootUntil||0,world.time+skill.root);e.phase='recover';e.timer=skill.root;}
 if(skill.slow){e.slowUntil=Math.max(e.slowUntil||0,world.time+.7);e.slow=Math.max(e.slow||0,skill.slow);}
 if(skill.dot){e.dots??=[];const prior=e.dots.find(d=>d.ownerId===p.id&&d.type===skill.dot.type);if(prior)prior.until=world.time+skill.dot.duration;else e.dots.push({...skill.dot,ownerId:p.id,until:world.time+skill.dot.duration,next:world.time+skill.dot.interval});}
}
function affectArea(world,p,center,skill){
 for(const e of world.enemies)if(e.hp>0&&distance(center,e)<=skill.radius&&hasLineOfSight(center,e,world.obstacles))hitEnemy(world,p,e,skill,skill.damage>0?skill.damage+p.state.level*2+p.state.damageBonus*.35:0);
 for(const ally of world.connected())if(!ally.state.ended&&distance(center,ally)<=skill.radius&&hasLineOfSight(center,ally,world.obstacles)){
...
export function castClassAbility(world,p,m){
 ...
 p.angle=a;world.emit('ability',{playerId:p.id,action:m.action,classId:p.state.classId,appearanceId:p.state.appearanceId,kind:skill.kind,color:skill.color||classColor(p.state),x:center.x,z:center.z,angle:a});
 const damage=(skill.damage||0)+p.state.level*2+p.state.damageBonus;
 if(skill.kind==='melee')for(let i=0;i<(skill.hits||1);i++)world.hits.push({playerId:p.id,at:world.time+.11+i*.13,angle:a,skill,damage});
 if(skill.kind==='projectile')for(let i=0;i<(skill.count||1);i++){
  ...
  world.projectiles.push({id:randomUUID(),x:p.x,z:p.z,angle,speed:skill.speed,damage,hostile:false,ownerId:p.id,life:skill.range/skill.speed,remainingHits:skill.pierce||1,hitIds:[],skill,visual:skill.projectile,color:skill.color||classColor(p.state)});
 }
 ...
 if(skill.kind==='zone')world.zones.push({id:randomUUID(),ownerId:p.id,classId:p.state.classId,color:classColor(p.state),action:m.action,x:center.x,z:center.z,radius:skill.radius,start:world.time,until:world.time+skill.duration,next:world.time,skill});
 if(skill.kind==='dodge'){
  let stepped=false;
  if(skill.shadowstep){const enemy=world.enemies.filter(e=>e.hp>0&&withinArc(p,e,a,skill.range,1.6)).sort((x,y)=>distance(p,x)-distance(p,y))[0];
   if(enemy){const destination={x:enemy.x-Math.sin(enemy.angle)*1.15,z:enemy.z-Math.cos(enemy.angle)*1.15};
    if(!pointBlocked(destination,world.obstacles,.42)&&hasLineOfSight(p,destination,world.obstacles,.42)&&destination.x>=WORLD_BOUNDS.minX&&destination.x<=WORLD_BOUNDS.maxX&&destination.z>=WORLD_BOUNDS.minZ&&destination.z<=WORLD_BOUNDS.maxZ){Object.assign(p,destination);p.dodge=0;p.input={x:0,z:0};stepped=true;}
   }
  }
...
export function resolveClassHits(world){
 for(const hit of world.hits)if(hit.at<=world.time){const p=world.players.get(hit.playerId);if(!p?.connected||p.state.ended)continue;const skill=hit.skill||abilitiesFor(p.state).attack;
  for(const e of world.enemies)if(e.hp>0&&hasLineOfSight(p,e,world.obstacles)&&withinArc(p,e,hit.angle,skill.range+(e.type==='boss'?.5:0),skill.arc))hitEnemy(world,p,e,skill,hit.damage??skill.damage+p.state.level*2+p.state.damageBonus);
 }
...
export function advanceClassEffects(world){
 for(const e of world.enemies){if((e.slowUntil||0)<=world.time)e.slow=0;
  for(const dot of e.dots||[]){const owner=world.players.get(dot.ownerId);if(e.hp>0&&owner?.connected&&!owner.state.ended&&dot.next<=world.time&&world.time<=dot.until){world.damageEnemy(e,dot.damage,dot.type==='poison',classColor(owner.state));dot.next+=dot.interval;}}
  e.dots=(e.dots||[]).filter(d=>d.next<=d.until&&d.until>=world.time&&world.players.get(d.ownerId)?.connected);
 }
 world.zones=world.zones.filter(zone=>{const owner=world.players.get(zone.ownerId);if(!owner?.connected||owner.state.ended||world.time>=zone.until)return false;
