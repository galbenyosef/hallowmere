...
export function createMultiplayerView({scene,camera,cloneModel,getRig,animateRig,animateHeroAttack,player}){
 ...
 function sync(players,you,time=performance.now()/1000,reset=false){
  reset=reset||latestTime!==null&&(time<latestTime||time-latestTime>.5);
  if(renderTime===null||reset||time-renderTime>.5)renderTime=time-INTERPOLATION_DELAY;
  latestTime=time;selfId=you;currentMap=players.find(p=>p.id===you)?.mapId||'overworld';const ids=new Set(players.map(p=>p.id));for(const [id,a] of actors)if(!ids.has(id)){a.label.remove();if(a.model!==player)disposeActor(a.model);a.ring.removeFromParent();a.ring.geometry.dispose();a.ring.material.dispose();actors.delete(id);}
  for(const p of players){let a=actors.get(p.id);if(!a){const model=p.id===you?player:cloneModel(conceptFor(p.classId,p.appearanceId));colorWarden(model,p.color);if(p.id!==you){model.position.set(p.x,0,p.z);scene.add(model);}
    ...
    a={model,label,ring,rig:getRig(model),motion:new MovementBuffer(),classKey:conceptFor(p.classId,p.appearanceId)};actors.set(p.id,a);
   }const classKey=conceptFor(p.classId,p.appearanceId);if(a.classKey!==classKey){if(p.id!==you){const replacement=cloneModel(classKey);replacement.position.copy(a.model.position);replacement.rotation.copy(a.model.rotation);disposeActor(a.model);a.model=replacement;scene.add(replacement);}a.rig=getRig(a.model);a.classKey=classKey;}if(a.motion.push(p,time,reset)&&p.id!==you){a.model.position.set(p.x,0,p.z);a.model.rotation.y=p.angle||0;}a.state=p;const name=p.classId?(classAppearance(p.classId,p.appearanceId)?.name||classFor(p).name):`Warden ${p.slot+1}`;a.label.textContent=p.id===you?`You · ${name}`:`${name}${p.classId?' · '+(p.slot+1):''}${p.ended?' · Fallen':''}`;
  }
 }
 function update(dt,t){if(renderTime!==null)renderTime=Math.min(renderTime+dt,latestTime);for(const [id,a] of actors){const p=a.state,sameMap=(p.mapId||'overworld')===currentMap;a.model.visible=sameMap;a.ring.visible=sameMap&&!p.ended;a.label.hidden=!sameMap;if(!sameMap)continue;if(id!==selfId){const blend=1-Math.exp(-dt*14),motion=a.motion.sample(renderTime),moving=Math.hypot(motion.x-a.model.position.x,motion.z-a.model.position.z)>.001;a.model.position.x=motion.x;a.model.position.z=motion.z;a.model.rotation.y=motion.angle;a.model.rotation.z=T.MathUtils.lerp(a.model.rotation.z,p.ended?-1.5:0,blend);animateRig(a.rig,t,moving);animateHeroAttack(a.rig,dt);}
