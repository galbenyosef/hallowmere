...
function lootCollected(drop,result){ctx.audio.play(drop.template==='bellkeeper-edge'?'relic':drop.kind==='item'?'equip':'pickup',.65);ctx.toast(drop.kind==='gold'?`+${drop.amount} crowns`:drop.kind==='item'?`${drop.name} recovered · Open inventory with I`:`${drop.amount} healing draught${drop.amount>1?'s':''} recovered`);if(drop.template==='bellkeeper-edge'&&!ctx.victoryShown){ctx.victoryShown=true;const worldId=ctx.network.worldId;ctx.victoryTimer=setTimeout(()=>{if(!ctx.state.ended&&ctx.renderedMap==='overworld'&&ctx.network.worldId===worldId)showModal('victory');},900);}ctx.updateUI();}
