...
async function init(){let loadingFailed=false;try{
 ...
 ctx.life=new VillageLife({scene:ctx.scene,camera:ctx.camera,player:ctx.player,state:ctx.state,cloneModel,reducedMotion:ctx.reducedMotion,obstacles:ctx.environment.obstacles,onTalk:talkTo,onCollect:lootCollected,onLootClick:collectClickedLoot,onApproach:point=>{awaken();return setDestination(point);}});
 mouseTargeting=createMouseTargeting({scene:ctx.scene,camera:ctx.camera,canvas:$('world'),enemies:ctx.enemies});
 ctx.multiplayerView=createMultiplayerView({scene:ctx.scene,camera:ctx.camera,cloneModel,getRig,animateRig,animateHeroAttack,player:ctx.player});
 ctx.rosterPicker=createRosterPicker({getState:()=>creatingJourney?{}:ctx.state,onChoose:chooseCharacter,onClose:closeRoster,onNavigate:navigateMenu});
 ...
 ctx.clock=new T.Clock();assetsReady=true;updateUI();drawMap();titleScreen.setProgress(100);showModeChoice();ctx.renderer.setAnimationLoop(frame);
...
function showModeChoice(){
 setModeChoiceInert(true);
 ctx.ready=false;sessionMode=null;mainMenuOpen=false;syncAudioState();
...
function openJourneys(preferredId){
 titleScreen.hide();setModeChoiceInert(true);ctx.ready=false;journeysMenu.show(preferredId);
}
function chooseJourneyCharacter(){creatingJourney=true;ctx.rosterPicker.show({journey:true});}
...
async function beginJourney(choice){
 ...
 try{
  ...
  creatingJourney=false;ctx.rosterPicker.resolve({ok:true});
 }catch(error){if(record){stopJourneySession();await journeyStore.release(record.id).catch(()=>{});}activeJourney=null;ctx.rosterPicker.resolve({ok:false,reason:error.message});}
}
...
function attachAutosave(record){
 autosave=createAutosave({store:journeyStore,record,getSave:()=>ctx.network.capture(),onStatus:updateSaveStatus,onConflict:error=>{
  journeyConflict=true;ctx.paused=true;releaseInput();ctx.audio.pause(true,ctx.backgrounded);
...
async function saveAndExit(){
 ...
 journeyLeaving=true;ctx.paused=true;releaseInput();
...
function stopJourneySession(){
 autosave?.stop();autosave=null;ctx.network?.close();ctx.network=null;sessionGeneration++;ctx.ready=false;ctx.started=false;
 activeJourney=null;sessionMode=null;ctx.lastSnapshot=null;ctx.lastNetworkEvent=0;mainMenuOpen=false;journeyConflict=false;
 clearTimeout(ctx.victoryTimer);clearTimeout(saveIndicatorTimer);ctx.victoryShown=false;cavePreviewStarted=false;
...
function startSession(mode,save){
 ...
 sessionMode=mode;ctx.ready=true;setModeChoiceInert(false);const generation=++sessionGeneration;
 const current=callback=>(...args)=>{if(generation===sessionGeneration)callback(...args);};
 const callbacks={onSnapshot:current(applySnapshot),onProgress:current(soon=>autosave?.changed(soon)),onStatus:current(connectionStatus),onWelcome:current(()=>{releaseInput();ctx.movementCorrection.reset();ctx.resetMovement=true;ctx.moveTarget=null;ctx.movePath=[];})};
 ctx.network=mode==='single-player'?new LocalSession({...callbacks,save}):new MultiplayerClient(callbacks);
 titleScreen.hide();ctx.clock.getDelta();ctx.network.start();awaken();
}
function returnToModeChoice(){
 if(ctx.lastSnapshot)return;
 sessionGeneration++;ctx.network?.close();ctx.network=null;releaseInput();showModeChoice();
}
function mainMenuSession(){
 return {mode:sessionMode,canChangeCharacter:!activeJourney&&!!ctx.network?.connected&&!ctx.state.ended&&ctx.safeHere(),character:classFor(ctx.state).name,characterLocked:!!activeJourney};
}
function openMainMenu(){
 ...
 mainMenuOpen=true;ctx.paused=true;releaseInput();ctx.audio.pause(true,ctx.backgrounded);
...
function resumeFromMainMenu(){
 ...
 if(!mainMenuOpen||!ctx.network?.connected||ctx.state.ended)return;
...
function closeRoster(){
 ...
 if(mainMenuOpen){ctx.paused=true;ctx.audio.pause(true,ctx.backgrounded);titleScreen.showMainMenu(mainMenuSession());return;}
...
window.addEventListener('pagehide',()=>{autosave?.emergency();autosave?.exit().catch(()=>{});ctx.network?.close();});
...
function cloneModel(name){if(!ctx.prefabs[name]){const definition=CLASS_LIST.find(c=>c.concept===name);ctx.prefabs[name]=createPlayableCharacter(definition?.id||'sorcerer',name);optimizeModel(ctx.prefabs[name]);}const model=ctx.prefabs[name].clone(true),materials=new Map();model.traverse(o=>{if(!o.isMesh)return;o.castShadow=true;o.receiveShadow=true;const cloneMaterial=source=>{if(!materials.has(source.uuid)){const m=source.clone();m.userData.baseEmissive=m.emissive.clone();materials.set(source.uuid,m);}return materials.get(source.uuid);};o.material=Array.isArray(o.material)?o.material.map(cloneMaterial):cloneMaterial(o.material);});return model;}
...
function spawnEnemy(type,x,z,zone='hallowmere',id=`enemy-${ctx.enemies.length}`){const data=ENEMY_TYPES[type],base=ctx.enemyModelType(type),name=base==='hound'?'grave-hound':base==='boss'?'bellkeeper':base;const model=cloneModel(name);if(data.modelType&&!data.tint){const tint=new T.Color(({hunter:'#a8baa2',rootling:'#7fa877',miner:'#bdaf93','quarry-mage':'#b99ad0',sentinel:'#c6a478',pyromancer:'#ea936d',rootbound:'#8eae78','quarry-warden':'#bfb1a0','ash-regent':'#edb085'})[type]||'#ffffff');model.traverse(o=>{if(o.material?.color)o.material.color.multiply(tint);});}const pickBounds=new T.Box3().setFromObject(model);model.position.set(x,0,z);model.rotation.y=Math.random()*6.28;ctx.scene.add(model);const e={id,zone,type,data,model,pickBounds,rig:getRig(model),hp:data.hp,maxHp:data.hp,phase:'idle',timer:0,cooldown:1+Math.random()*1.5,angle:0,attackAngle:0,aim:new T.Vector3(),flash:0,dead:false,home:{x,z},attackCount:0,seed:Math.random()*6.28,telegraph:null,path:[],navTimer:0,bar:null,barCanvas:null,barTexture:null,barHealth:data.hp,recoil:0};
...
function updatePointer(event){const rect=$('world').getBoundingClientRect();ctx.pointer.set((event.clientX-rect.left)/rect.width*2-1,-(event.clientY-rect.top)/rect.height*2+1);ctx.aimActive=true;mouseInWorld=true;pointerIsMouse=event.pointerType!=='touch';pointerShift=event.shiftKey;if(ctx.ready){getPointerWorld();updateMouseTarget();}}
...
function pointerAction(){
 ...
 const enemy=mouseTargeting.pick(ctx.pointer,{assist:pointerIsMouse})||(!pointerIsMouse?enemyAtPoint(ctx.targetWorld):null);
...
function attacksFromHere(){return abilitiesFor(ctx.state).attack.kind==='projectile'||pointerShift;}
...
function toggleMap(){
 ...
 if(ctx.mapExpanded){panel.setAttribute('role','dialog');panel.setAttribute('aria-modal','true');panel.setAttribute('aria-labelledby','map-title');updateMenuNavigation($('map-navigation'),'map',{canChangeCharacter:!activeJourney&&ctx.safeHere(),characterLocked:!!activeJourney});$('map-button').focus({preventScroll:true});}
...
function perform(action){
 ...
 const targetEnemy=offensive?(ctx.attackHeld&&ctx.lockedEnemy&&!ctx.lockedEnemy.dead?ctx.lockedEnemy:mouseTargeting?.selected):null;
...
function updateClassHud(){
 const c=classFor(ctx.state),key=conceptFor(ctx.state.classId,ctx.state.appearanceId)+'|'+!!activeJourney;if(hudClass===key)return;hudClass=key;
...
function syncPlayerCharacter(){
 if(ctx.state.classId&&!portraitsRequested){portraitsRequested=true;prepareCharacterPortraits().then(()=>{if(ctx.modalKind==='inventory')renderInventory();}).catch(error=>console.error('Character portraits unavailable',error));}
 const key=conceptFor(ctx.state.classId,ctx.state.appearanceId);if(ctx.player.userData.characterKey===key)return;
 for(const child of [...ctx.player.children])disposeActor(child);ctx.player.add(cloneModel(key));ctx.player.userData.characterKey=key;ctx.heroRig=getRig(ctx.player);
}
function openRoster(){
 ...
 if(!ctx.safeHere()){if(mainMenuOpen)titleScreen.updateSession(mainMenuSession());else toast('Return to a sanctuary to change class.');return;}
...
function showModal(kind){if(mainMenuOpen&&kind!=='death')return;if(!ctx.ready||!ctx.network?.connected||ctx.mapExpanded||ctx.state.ended&&kind!=='death')return;inventoryPreviews.hide();if(kind!=='death'&&kind!=='victory')ctx.audio.play('ui-open',.55);if(!ctx.modalKind)ctx.previousFocus=document.activeElement;ctx.modalKind=kind;ctx.paused=true;releaseInput();ctx.audio.pause(true,ctx.backgrounded);$('modal-shade').hidden=false;$('modal-secondary').hidden=true;$('inventory-purse').hidden=kind!=='inventory';$('inventory-controls').hidden=kind!=='inventory';document.querySelector('.modal').classList.toggle('inventory-modal',kind==='inventory');document.querySelector('.modal').classList.toggle('pause-modal',kind==='pause');document.querySelector('.modal').classList.toggle('npc-modal',kind==='npc');$('modal-shade').classList.toggle('npc-conversation',kind==='npc');$('modal-primary').hidden=kind==='npc'||kind==='pause';$('dialogue-portrait').hidden=true;document.querySelector('.modal').classList.remove('has-npc-portrait');$('modal-eyebrow').textContent=kind==='victory'?'THE BELLKEEPER DEFEATED':kind==='death'?'THE VEIL TAKES ANOTHER':'THE ASHEN VIGIL';const title=$('modal-title'),content=$('modal-content'),primary=$('modal-primary');
 ...
 if(kind==='pause'){title.textContent=sessionMode==='single-player'?'Game paused':'Game menu';content.innerHTML=pauseMenuMarkup(ctx.gameSettings,{journey:!!activeJourney});if(activeJourney){autosave?.changed();autosave?.flush().catch(()=>{});updateSaveStatus();}}
 ...
 if(kind==='death'){title.textContent='Return to the lanterns';content.innerHTML=`<p>${sessionMode==='multiplayer'?'Your allies continue the fight.<br>':''}Return to ${CHECKPOINTS.find(c=>c.id===ctx.state.checkpointId)?.name||'Ashwick'} with your equipment intact.</p><div class="victory-stats"><div><strong>${ctx.state.kills}</strong><span>SLAIN</span></div><div><strong>${ctx.state.souls}</strong><span>SOULS</span></div></div>`;primary.textContent='Respawn at checkpoint';if(activeJourney)content.insertAdjacentHTML('beforeend','<button type="button" class="text-button" data-save-exit>Save &amp; exit</button><span class="journey-save-status" data-save-status role="status"></span>');}
...
function closeModal(){if(journeyLeaving||journeyConflict)return;if(ctx.state.ended||ctx.rosterPicker?.open)return;if(mainMenuOpen){resumeFromMainMenu();return;}inventoryPreviews.hide();ctx.audio.play('ui-close',.5);$('modal-shade').hidden=true;ctx.paused=false;syncAudioState();ctx.modalKind='';ctx.currentNpc=null;ctx.previousFocus?.focus?.({preventScroll:true});$('world').focus({preventScroll:true});}
...
function restart(){if(!ctx.network?.connected)return;if(ctx.state.ended){ctx.network.send('respawn');}else{ctx.network.send(sessionMode==='single-player'?'restart':'vote',{agree:true});closeModal();}}
...
function applySnapshot(snapshot,changed){
 ...
 ctx.exploration.setSession(snapshot.worldId,snapshot.you,{mode:sessionMode,preview:ctx.previewMode,journeyId:activeJourney?.id});
 if(changed&&sessionMode==='single-player')ctx.exploration.reset();
 ...
 if(mainMenuOpen&&!ctx.rosterPicker?.open)titleScreen.updateSession(mainMenuSession());
...
function networkEvent(event){
 if(event.type==='result'&&event.operation==='class'&&ctx.state.classId&&sessionMode==='single-player'&&!cavePreviewStarted&&new URLSearchParams(location.search).get('preview')==='enemies'){cavePreviewStarted=true;const generation=sessionGeneration;import('./enemy-preview.js').then(({createEnemyPreview})=>{if(generation===sessionGeneration)createEnemyPreview(ctx.network,()=>{releaseInput();ctx.resetMovement=true;});});}
 if(event.type==='result'&&event.operation==='class'){ctx.rosterPicker?.resolve(event);if(ctx.state.classId&&sessionMode==='single-player'&&!cavePreviewStarted&&['caves','exploration'].includes(new URLSearchParams(location.search).get('preview'))){cavePreviewStarted=true;const generation=sessionGeneration;import('./cave-preview.js').then(({createCavePreview})=>{if(generation===sessionGeneration)createCavePreview(ctx.network,()=>{releaseInput();ctx.resetMovement=true;});});}}
...
function renderSharedWorld(dt,t){renderHazards();ctx.classEffects.syncActors(ctx.lastSnapshot?.players||[],id=>id===ctx.network?.id?ctx.player:ctx.multiplayerView?.actors.get(id)?.model,ctx.renderedMap);
 for(const e of ctx.enemies){const n=e.net;if(!n)continue;const blend=1-Math.exp(-dt*14);e.model.position.x=T.MathUtils.lerp(e.model.position.x,n.x,blend);e.model.position.z=T.MathUtils.lerp(e.model.position.z,n.z,blend);e.model.rotation.y=angleLerp(e.model.rotation.y,n.angle,blend);
  ...
  if(e.barHealth>e.hp){e.barHealth=Math.max(e.hp,e.barHealth-dt*e.maxHp*1.6);updateEnemyBar(e);}animateRig(e.rig,t,n.moving,n.phase==='windup'?1-n.timer/e.data.windup:0,ctx.enemyModelType(e.type));e.visuals.update(n,dt,t+e.seed);e.bar.position.copy(e.model.position).add(new T.Vector3(0,(ctx.enemyModelType(e.type)==='boss'?4.8:ctx.enemyModelType(e.type)==='hound'?1.5:ctx.enemyModelType(e.type)==='hollow'?2.25:3.25)*(e.data.scale||1),0));e.bar.visible=e===mouseTargeting?.selected||distance(ctx.player.position,e.model.position)<12;
...
window.hallowmere={getState:()=>({...ctx.state,cooldowns:{...ctx.state.cooldowns},ready:ctx.ready,paused:ctx.paused,backgrounded:ctx.backgrounded,mode:sessionMode,simulationPaused:!ctx.network?.connected||sessionMode==='single-player'&&(ctx.paused||ctx.backgrounded||!!ctx.rosterPicker?.open),multiplayer:{connected:sessionMode==='multiplayer'&&!!ctx.network?.connected,worldId:ctx.network?.worldId,players:ctx.lastSnapshot?.players||[],votes:ctx.lastSnapshot?.votes||[]},modal:ctx.modalKind,interior:ctx.player?ctx.environment.currentBuilding(ctx.player.position)?.id??null:null,buildings:ctx.environment?.buildings.map(b=>({id:b.id,name:b.name,chapel:b.chapel,locked:!b.doorCollider.disabled,inside:b.inside,door:{...b.door},entry:{...b.entry},exit:{...b.exit}})),dialogue:ctx.modalKind==='npc'?npcDialogue(ctx.state,ctx.currentNpc):null,objective:questSummary(ctx.state),mapId:ctx.renderedMap,bounds:ctx.worldBounds(),interactions:regionInteractions(),hazards:ctx.lastSnapshot?.hazards||[],audio:ctx.audio.getState(),targeting:{hoveredEnemy:mouseAction?.kind==='enemy'?mouseAction.target.id:null,lockedEnemy:ctx.lockedEnemy?.id??null,highlightedEnemy:mouseTargeting?.selected?.id??null},player:ctx.player?{x:ctx.player.position.x,z:ctx.player.position.z,moving:!!ctx.moveTarget,attacking:ctx.attackHeld}:null,...ctx.life?.getState(),enemies:ctx.enemies.filter(e=>!e.dead).map(e=>({id:e.id,type:e.type,zone:e.zone,hp:e.hp,x:e.model.position.x,z:e.model.position.z,phase:e.phase,lineOfSight:hasLineOfSight(ctx.player.position,e.model.position,ctx.environment.obstacles)})),drawCalls:ctx.renderer?.info.render.calls}),showControls:()=>showModal('help'),pause:()=>showModal('pause'),resume:closeModal};
