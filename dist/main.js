...
function perform(action){
 ...
 if(!aimActive&&action!=='dodge'){const target=lockedEnemy&&!lockedEnemy.dead?lockedEnemy:nearestEnemy(Math.max(7,abilitiesFor(state)[action]?.range||0));if(target)angle=Math.atan2(target.model.position.x-player.position.x,target.model.position.z-player.position.z);}
 if(!network.send('ability',{action,angle,...(aimActive?{target:{x:targetWorld.x,z:targetWorld.z}}:lockedEnemy?{target:{x:lockedEnemy.model.position.x,z:lockedEnemy.model.position.z}}:{})}))return false;
...
function updateUI(){if(!player)return;updateClassHud();$('health-liquid').style.height=`${state.hp/state.maxHp*100}%`;$('mana-liquid').style.height=`${state.mana/state.maxMana*100}%`;$('potion-count').textContent=state.potions;$('souls-counter').textContent=`${state.souls} SOULS`;$('experience-fill').style.width=`${state.souls%100}%`;$('level-label').textContent=`${classAppearance(state.classId,state.appearanceId)?.name||classFor(state).name} · LEVEL ${state.level}`;document.querySelector('.rank').textContent=String(state.level).padStart(2,'0');
 ...
 $('enemy-target').hidden=!aimActive||!!boss;if(aimActive&&!enemies.some(e=>!e.dead&&bossType(e.type))){const e=pickEnemy();$('enemy-target').hidden=!e;if(e){$('target-name').textContent=e.data.name;$('target-type').textContent=e.data.attackStyle==='orb'?'THE AFFLICTED · CASTER':e.data.attackStyle==='bite'?'THE AFFLICTED · DEVOURER':'THE AFFLICTED · KNIFE';$('target-fill').style.width=`${Math.max(0,e.hp/e.maxHp*100)}%`;}}
