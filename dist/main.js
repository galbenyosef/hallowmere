...
$('world').addEventListener('pointerdown',event=>{
 ...
 if(event.button===2){perform('bolt');return;}if(event.button!==0)return;
 ...
 if(lockedEnemy){
  attackHeld=true;moveTarget=null;movePath=[];life.pending=null;pendingRegionInteraction=null;
  angle=Math.atan2(lockedEnemy.model.position.x-player.position.x,lockedEnemy.model.position.z-player.position.z);
  if(distance(player.position,lockedEnemy.model.position)<=abilitiesFor(state).attack.range)perform('attack');
 }else if(event.shiftKey||distance(player.position,targetWorld)<2.4){moveTarget=null;movePath=[];attackHeld=true;perform('attack');}
 else setDestination(targetWorld);
 updateMouseTarget();
});
function releaseMouseAttack(){if(lockedEnemy)movePath=[];attackHeld=false;lockedEnemy=null;updateMouseTarget();}
