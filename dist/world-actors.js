...
export class VillageLife{
 ...
 nearest(){const nearby=[...this.drops,...this.forage].filter(d=>!d.claimed&&distance(d.model.position,this.player.position)<2.8&&(d.kind!=='potion'||this.state.potions<5)&&hasLineOfSight(d.model.position,this.player.position,this.obstacles)).sort((a,b)=>(b.rarity==='legendary')-(a.rarity==='legendary')||distance(a.model.position,this.player.position)-distance(b.model.position,this.player.position));if(nearby.length)return nearby[0];return this.visibleNpcs().filter(n=>distance(n.model.position,this.player.position)<3.3&&hasLineOfSight(n.model.position,this.player.position,this.obstacles)).sort((a,b)=>distance(a.model.position,this.player.position)-distance(b.model.position,this.player.position))[0]||null;}
