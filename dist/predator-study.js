...
const controls = [...viewButtons, ...poseButtons, ...appearanceButtons, $('rotate-left'), $('rotate-right'), $('reset-view')];
const views = {
  'three-quarter': {label:'Three-quarter view',rotation:-.34,eye:[0,3.3,9],target:[0,1.40,0],span:3.50},
  front: {label:'Front view',rotation:0,eye:[0,1.4,9],target:[0,1.4,0],span:3.45},
  side: {label:'Side view',rotation:-Math.PI/2,eye:[0,1.4,9],target:[0,1.4,0],span:3.45},
  back: {label:'Back view',rotation:Math.PI,eye:[0,1.4,9],target:[0,1.4,0],span:3.45},
  face: {label:'Head & tendrils',rotation:-.18,eye:[0,2.39,7],target:[0,2.39,0],span:1.29},
  game: {label:'Isometric game view',rotation:0,eye:[17,25,26],target:[0,1.35,0],span:9},
  compare: {label:'Hunter & king comparison',rotation:-.16,eye:[0,3.9,10],target:[0,1.63,0],span:4.1}
};
const poses = {
  stalk: ['Stalking stance','Both arms are at rest with the wrist blades retracted. Inspect the armor, netting, and tendrils.'],
  blades: ['Wrist blades extended','Two long wrist blades extend from the right gauntlet. The other hand remains free.'],
  aim: ['Shoulder cannon ready','The shoulder cannon turns forward with a charged plasma lens. This is a still equipment preview.']
};
const appearances = {
  masked: 'Ridged metal mask, dark eye slits, and a narrow breathing grille.',
  unmasked: 'Amber eyes and mottled skin frame an open mouth with four tusked mandibles.'
};
let renderer,scene,camera,hunter,king,pedestal,frame=0,activeView='three-quarter',stopped=false,pointer=null,observer;
const geometries=[],materials=[];
controls.forEach(button=>button.disabled=true);

function createPedestal() {
  const root=new T.Group(); root.name='hunter-study-platform';
...
function updateCamera() {
  ...
  if(activeView==='face')span=Math.max(span,1.3/aspect);
    ...
    hunter.root.updateMatrixWorld(true);king.root.updateMatrixWorld(true);
    const bounds=new T.Box3().setFromObject(hunter.root);
    if(king.root.visible)bounds.union(new T.Box3().setFromObject(king.root));
...
function setView(view) {
  activeView=view;hunter.root.rotation.y=views[view].rotation;king.root.rotation.y=views[view].rotation;
  const comparison=view==='compare';
  king.root.visible=comparison;hunter.root.position.x=comparison?-.85:0;king.root.position.x=1.0;
  pedestal.scale.x=comparison?2:1;
  $('view-label').textContent=views[view].label;
  $('scale-indicator').textContent=comparison?'Shared world scale':view==='face'?'Head detail':view==='game'?'Game-scale readability':'Character detail';
  viewButtons.forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.view===view)));
  updateCamera();
}
function rotate(amount) {
  if(stopped)return;
  hunter.root.rotation.y+=amount;king.root.rotation.y+=amount;
  $('view-label').textContent=activeView==='compare'?'Hunter & king · custom angle':'Custom angle';
  viewButtons.forEach(button=>button.setAttribute('aria-pressed','false'));updateCamera();
}
function setPose(pose) {
  hunter.setPose(pose);poseButtons.forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.pose===pose)));
  $('pose-label').textContent=poses[pose][0];$('equipment-caption').textContent=poses[pose][1];updateCamera();
}
function setAppearance(appearance) {
  hunter.setAppearance(appearance);
  appearanceButtons.forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.appearance===appearance)));
  $('appearance-caption').textContent=appearances[appearance];
  canvas.setAttribute('aria-label',`Predator-inspired hunter, ${appearance}, with banded tendrils, wrist blades, and a shoulder cannon`);
  updateCamera();
}
...
function dispose() {
  stopped=true;cancelAnimationFrame(frame);frame=0;observer?.disconnect();hunter?.dispose();king?.dispose();
...
try {
  ...
  hunter=createPredatorCharacter();king=createHuntKingCharacter();king.setPose('guard');king.root.visible=false;scene.add(hunter.root,king.root);
...
}catch(error){console.error('Predator study could not initialize:',error);dispose();fail('The 3D preview could not start. Enable WebGL and reload this page. The design notes remain available.');}

viewButtons.forEach(button=>button.addEventListener('click',()=>setView(button.dataset.view)));
poseButtons.forEach(button=>button.addEventListener('click',()=>setPose(button.dataset.pose)));
appearanceButtons.forEach(button=>button.addEventListener('click',()=>setAppearance(button.dataset.appearance)));
$('rotate-left').addEventListener('click',()=>rotate(-Math.PI/6));$('rotate-right').addEventListener('click',()=>rotate(Math.PI/6));
$('reset-view').addEventListener('click',()=>{setPose('blades');setAppearance('masked');setView('three-quarter');});
...
canvas.addEventListener('webglcontextlost',event=>{event.preventDefault();fail('The 3D view lost its graphics connection. Reload this page to restore the hunter.');});
