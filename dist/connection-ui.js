...
// ctx.leaveSession/ctx.startSession (M9's createSessionLifecycle), read lazily inside the
...
export function createConnectionUi(ctx){
 ...
 $('connection-retry').onclick=()=>{if(ctx.lastSnapshot){location.reload();return;}if(ctx.leaveSession())ctx.startSession('multiplayer');};
