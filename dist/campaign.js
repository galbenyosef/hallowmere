...
export function regionQuestSummary(state){
 ...
 if(map.theme==='cave'){const found=map.caches.filter(c=>state.claimedCaches?.includes(c.id)).length,claimed=found===map.caches.length;return{title:map.name,objective:claimed?'Caches claimed · Return to Hallowmere':`Explore the tunnels · ${found} / ${map.caches.length} caches`,count:claimed?'✓':'OPTIONAL',hint:claimed?'Use the entrance to return to the surface.':'Follow the lanterns through branching passages. Defeat each cache’s guardians to claim its treasure.'};}
