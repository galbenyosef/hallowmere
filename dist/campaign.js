...
export function npcDialogue(state,id){const npc=NPCS.find(n=>n.id===id);if(!npc)return null;let text='',choices=[];
 if(id==='rowan'){if(state.victory&&state.bossLootClaimed&&!state.questRewarded){text='No bell. Only the wind. You have brought peace to Hallowmere, traveler. Take this with the thanks of both villages.';choices=[{action:'claim-reward',response:'You have my thanks.',detail:'The Last Toll · Quest complete',priceLabel:'+80 crowns',label:'Accept the village’s thanks · 80 crowns'}];}else if(state.questRewarded){text='The road is ours again. You will always find a warm hearth in Ashwick.';}else if(!state.questAccepted){text='Hallowmere’s bell has rung for thirteen years. The afflicted haunt the road between our villages. Follow the old stones east, speak to Watchman Rook, then cleanse the village and silence the Bellkeeper.';choices=[{action:'accept-quest',response:'I will silence the bell.',detail:'Accept quest · The Last Toll',label:'Accept · The Last Toll'}];}else{text='The Mourning Road runs east. Rook waits at Hallowmere’s gate. Clear the twelve afflicted inside the village; the Bellkeeper will answer. Bring his relic back to us.';}}
 if(id==='edda'){text='The lanterns keep the afflicted outside our ward. Rest here. I can mend your wounds and refill your draughts before you return to the road.';choices=[{action:'rest',response:'I could use some rest.',detail:'Restore vitality, essence, and draughts',priceLabel:'Free',label:'Rest and replenish · Free'}];}
 if(id==='brann'){text='A good weapon earns its keep on that road. Bring me crowns from the afflicted and I’ll improve your equipment. Any weapon you equip keeps my work.';choices=[{action:'forge',response:state.forgeLevel>=3?'My weapon is fully honed.':'Hone my weapon.',detail:'+4 damage · Permanent upgrade',cost:state.forgeLevel>=3?undefined:30*(state.forgeLevel+1),disabledReason:state.forgeLevel>=3?'+12 damage · Maximum upgrade':state.gold<30*(state.forgeLevel+1)?`Need ${30*(state.forgeLevel+1)-state.gold} more crowns · +4 damage`:'',label:state.forgeLevel>=3?'Weapon fully honed':`Hone your weapon · +4 damage · ${30*(state.forgeLevel+1)} crowns`,disabled:state.forgeLevel>=3||state.gold<30*(state.forgeLevel+1)},{action:'buy-potion',response:'I’ll take a healing draught.',detail:'+1 draught · Healing supplies',cost:12,disabledReason:state.potions>=5?'Draught belt is full':state.gold<12?`Need ${12-state.gold} more crowns`:'',label:'Buy a healing draught · 12 crowns',disabled:state.gold<12||state.potions>=5}];}
 if(id==='rook'){text=state.victory?'The Bellkeeper is gone. I can hear birds beyond the graves. Take your spoils back to Rowan; he will want to know.':'Ashwick is behind you. Hallowmere lies ahead. Twelve afflicted still prowl its lanes. When the last falls, the Bellkeeper will leave his chapel. Watch the red marks on the ground; move before he strikes.';if(!state.rookSupplies)choices.push({action:'take-supplies',response:'I’ll take those supplies.',detail:`+${Math.min(2,Math.max(0,5-state.potions))} draught${state.potions===4?'':'s'}`,priceLabel:'Free',disabledReason:state.potions>=5?'Draught belt is full':'',label:'Take Rook’s supplies · 2 draughts',disabled:state.potions>=5});if(!state.questAccepted)choices.push({action:'accept-quest',response:'I will silence the bell.',detail:'Accept quest · The Last Toll',label:'I will silence the bell'});}
