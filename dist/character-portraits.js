...
import {CLASS_LIST,SORCERER_APPEARANCES,conceptFor} from './classes.js';
...
async function build(){
 ...
 try{for(const [classId,appearanceId] of choices){
  const id=conceptFor(classId,appearanceId),study=createStudyScene(roster.find(c=>c.id===id));
  renderer.render(study.scene,camera);portraits.set(id,renderer.domElement.toDataURL('image/png'));
  const model=createPlayableCharacter(classId,appearanceId),weapon=model.getObjectByName('weapon');
  ...
  const geometry=new Set(),materials=new Set();model.traverse(n=>{if(n.geometry)geometry.add(n.geometry);if(n.material)materials.add(n.material);});geometry.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());disposeStudy(study);
