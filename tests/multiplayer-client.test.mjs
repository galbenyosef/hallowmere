...
test('transport waits for a snapshot, resumes per-tab identity, reconciles acknowledgments and blocks disconnected actions',async()=>{
 ...
 class Socket{constructor(url){this.url=url;this.readyState=1;this.sent=[];Socket.latest=this;}send(s){this.sent.push(JSON.parse(s));}close(){this.readyState=3;this.onclose?.();}receive(m){this.onmessage({data:JSON.stringify({v:1,...m})});}}
