...
// Usage: bash scripts/node22.sh node scripts/perf-browser.mjs [--runs 3] [--json] [--url http://127.0.0.1:5182]
...
import {withBrowser, playTo, evaluate, tool} from './screenshot.mjs';

const args=process.argv.slice(2);
const value=(name,fallback)=>{const i=args.indexOf(name);return i<0?fallback:args[i+1];};
const RUNS=Number(value('--runs',3)),JSON_OUT=args.includes('--json');
...
const report={runs:RUNS,renderer:runs[0].renderer,idleFrames:IDLE_FRAMES,walkFrames:WAYPOINTS.length*WALK_FRAMES,waypoints:WAYPOINTS,
 medians:{
...
console.log(`renderer ${report.renderer} | ${RUNS} runs | medians across runs`);
